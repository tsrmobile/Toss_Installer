package th.co.thiensurat.toss_installer.payment.paymentfragment.paymentjob;
...
public class PaymentFragment extends BaseMvpFragment<PaymentInterface.Presenter>
    ...
    public void itemClick(View view, int position) {
        ...
        if (jobItem.getProduct().size() > 1) {
            if (Utils.diffPayType(jobItem.getProduct()) == 0) {
                ...
                //getActivity().startActivityForResult(intent, Constance.REQUEST_PAYMENT);
                ...
                intent.putExtra(Constance.KEY_JOB_ADDR, addressItemGroup);
                intent.putExtra(Constance.KEY_JOB_PRODUCT, productItemGroup);
                //getActivity().startActivityForResult(intent, Constance.REQUEST_PAYMENT_ITEM_LIST);
                getActivity().startActivity(intent);
            }
        } else {
            Intent intent = new Intent(getActivity(), PaymentPageActivity.class);
